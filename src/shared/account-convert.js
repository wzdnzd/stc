...
export function getAuthDefaults(sourceFormat, account = {}) {
  const source = String(sourceFormat || "").toLowerCase();
  const type = String(account.type || account.provider || account.platform || "").toLowerCase();
  let clientId = account.client_id || account._client_id || account.clientId;

  // 如果 JSON 里没有 client_id，就动态生成
  if (!clientId) {
...
export function normalizeCpa(raw) {
  ...
  const source = detectSourceFormat(raw);
  const defaults = getAuthDefaults(source, raw);
...
export function cpaToSub2(cpaInput, options = {}) {
  ...
  const source = detectSourceFormat(cpaInput);
  const defaults = getAuthDefaults(source, cpaInput);
...
export function sub2ToCpa(sub2Input, options = {}) {
  const opts = normalizeConvertOptions(options);
  const source = detectSourceFormat(sub2Input);
  const defaults = getAuthDefaults(source, sub2Input);
...
export function finalizeCpa(c, options = {}) {
  const opts = normalizeConvertOptions(options);
  const source = detectSourceFormat(c);
  const defaults = getAuthDefaults(source, c);
...
export function convertAccountTo(account, sourceFormat, target, options = {}) {
  ...
  const authDefaults = getAuthDefaults(source, account);
